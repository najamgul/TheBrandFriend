...
export async function seedTopicsIfEmpty() {
  ...
  const { data, error } = await supabase
    .from('blog_topics')
    .upsert(rows, { onConflict: 'primary_keyword', ignoreDuplicates: true })
    .select('id');

  if (error) throw new Error(`Seeding topics failed: ${error.message}`);
...
export async function getPublishedPosts({ limit = 200 } = {}) {
  ...
  const { data, error } = await supabase
    .from('blog_posts')
    .select(
      'slug, seo_title, meta_description, tldr, category, tags, cover_image, cover_image_alt, author, published_at, updated_at, word_count'
    )
    .eq('status', 'published')

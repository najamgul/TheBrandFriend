...
export const SITE_URL = 'https://thebrandfriend.com';
const HOST = 'thebrandfriend.com';
...
/**
 * Requires a key file served at https://thebrandfriend.com/<key>.txt whose
...
export async function pingGoogleIndexing(urls) {
  ...
  try {
    ...
    return { ok: true, status: 'sent', count: urls.length };
  } catch (err) {
    return { ok: false, status: 'failed', message: err.message };
  }
}
...
export async function notifySearchEngines(urls) {
  ...
  return { indexnow: indexnow.status, google: googleResult.status };
}

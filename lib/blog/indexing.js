  ...
  try {
    ...
    const results = await Promise.allSettled(
      urls.map(async url => {
        ...
        // Reject, so the per-URL bookkeeping below still counts this as a
        // failure — the SDK threw on a non-2xx, fetch does not.
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error(
            `${res.status} ${body.error?.message || res.statusText || 'unknown'}`
          );
        }
